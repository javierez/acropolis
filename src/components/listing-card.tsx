...
export const PropertyCard = React.memo(function PropertyCard({
  ...
  const descriptionText =
    ...
    `${typeLabel}${((listing.squareMeter ?? 0) > 0 || (listing.builtSurfaceArea && Number(listing.builtSurfaceArea) > 0)) ? ` de ${formatNumber(listing.squareMeter || Math.round(Number(listing.builtSurfaceArea)))} m²` : ""}${!hideRooms && listing.bedrooms != null && listing.bedrooms > 0 ? ` con ${listing.bedrooms} ${useEstancias ? "estancias" : "habitaciones"}` : ""}`;
  ...
  const hasArea =
    ...
    (!!listing.builtSurfaceArea && Number(listing.builtSurfaceArea) > 0);
    ...
    <>
      ...
      {hasArea && (
        <div className="flex items-center">
          ...
          <span>
            {formatNumber(
              listing.squareMeter || Math.round(Number(listing.builtSurfaceArea)),
            )}{" "}
            m²
          </span>
        </div>
